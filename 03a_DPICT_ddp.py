...
Single GPU:
    - python 03_DPICT.py --dataset "datasets/ImageNet/val" --save_dir "results/03_DPICT" --seed 42 --batch_size 1 --cutoff_stride 20 --num_images 50000 --model_path "pretrained_weights/DPICT/000.pth.tar"

Multi GPU (DDP):
    - srun --gres=gpu:4 torchrun --nproc_per_node=4 03a_DPICT_ddp.py --distributed --dataset "datasets/ImageNet/val" --save_dir "results/03a_DPICT_ddp" --seed 42 --batch_size 1 --cutoff_stride 8 --num_images 50000 --model_path "pretrained_weights/DPICT/000.pth.tar"
'''
...
def ddp_is_on(args):
    return bool(args.distributed) and ("RANK" in os.environ) and ("WORLD_SIZE" in os.environ)
...
def ddp_init(args):
    if not ddp_is_on(args):
        return

    torch.cuda.set_device(ddp_local_rank())
    dist.init_process_group(backend=args.dist_backend, init_method="env://")

def ddp_barrier(args):
    if ddp_is_on(args):
        dist.barrier()

def ddp_broadcast_object(args, obj, src=0):
    if not ddp_is_on(args):
        return obj
    obj_list = [obj] if ddp_rank() == src else [None]
    dist.broadcast_object_list(obj_list, src=src)
    return obj_list[0]

def ddp_allreduce_sum(args, x: torch.Tensor):
    if ddp_is_on(args):
...
def tensor_to_pil(x01: torch.Tensor) -> Image.Image:
    x01 = x01.detach().clamp(0, 1)
    x01 = x01.squeeze(0).cpu()
...
def save_recon_image(
    ...
    im = tensor_to_pil(x_rec)

    if image_format.lower() in ["jpg", "jpeg"]:
        im.save(out_path, format="JPEG", quality=int(jpeg_quality), subsampling=0, optimize=True)
    else:
        im.save(out_path, format="PNG", compress_level=3)
...
class IndexedSubset(torch.utils.data.Dataset):
    def __init__(self, base_folder: ImageFolder, indices: List[int]):
    ...
    def __getitem__(self, i):
        ...
        path, _ = self.base.samples[base_idx]
        return x, y, i, path
...
def arg_parse(argv):
    ...
    p.add_argument("--image_format", type=str, default="png", choices=["png", "jpg", "jpeg"])
...
def main(args):
    ...
    folder = ImageFolder(root=args.dataset, transform=img_transform)
    max_images = min(args.num_images, len(folder))
    indices = list(range(max_images))
    dataset = IndexedSubset(folder, indices)
    ...
    if ddp_rank() == 0:
        print(f"[info] dataset={args.dataset} | images={len(dataset)} | batches={len(dataloader)}")
    ...
    downstream = build_downstream(device, args.dataset, folder.classes)
    if ddp_rank() == 0:
    ...
    it = dataloader
    if ddp_rank() == 0:
        it = tqdm(dataloader, desc="[DPICT]")

    with torch.no_grad():
        for x, y, idx_in_subset, path in it:
            ...
            for bi in range(B):
                ...
                if cutoff_list is None:
                    ...
                    cutoff_list = ddp_broadcast_object(args, cutoff_list, src=0)

                    if args.save_images and ddp_rank() == 0:
                        for c in cutoff_list:
                            ensure_dir(os.path.join(args.save_dir, f"recon_cutoff{c:03d}"))
                    ddp_barrier(args)

                class_name = folder.classes[int(y1.item())]
                base_name = os.path.basename(path1)
                stem, _ext = os.path.splitext(base_name)
                out_ext = "jpg" if args.image_format in ["jpg", "jpeg"] else "png"
                ...
                for cutoff in cutoff_list:
                    ...
                    if args.save_images:
                        out_dir = os.path.join(args.save_dir, f"recon_cutoff{cutoff:03d}", class_name)
                        out_path = os.path.join(out_dir, f"{idx1:05d}_{stem}.{out_ext}")
                        save_recon_image(
                            x_rec=x_rec,
                            out_path=out_path,
    ...
    if ddp_is_on(args):
        for cutoff in cutoff_list:
            t = torch.tensor([
                codec_stats[cutoff]["bpp"],
                codec_stats[cutoff]["psnr"],
                codec_stats[cutoff]["msssim"],
                codec_stats[cutoff]["N"],
            ], device=device, dtype=torch.float64)
            t = ddp_allreduce_sum(args, t)
            codec_stats[cutoff]["bpp"] = float(t[0].item())
            codec_stats[cutoff]["psnr"] = float(t[1].item())
            codec_stats[cutoff]["msssim"] = float(t[2].item())
            codec_stats[cutoff]["N"] = float(t[3].item())

            for mname in downstream.keys():
                t2 = torch.tensor([
                    cls_stats[cutoff][mname]["loss"],
                    cls_stats[cutoff][mname]["top1"],
                    cls_stats[cutoff][mname]["top5"],
                    cls_stats[cutoff][mname]["N"],
                ], device=device, dtype=torch.float64)
                t2 = ddp_allreduce_sum(args, t2)
                cls_stats[cutoff][mname]["loss"] = float(t2[0].item())
                cls_stats[cutoff][mname]["top1"] = float(t2[1].item())
                cls_stats[cutoff][mname]["top5"] = float(t2[2].item())
                cls_stats[cutoff][mname]["N"] = float(t2[3].item())
